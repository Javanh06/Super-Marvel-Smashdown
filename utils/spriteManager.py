...
class SpriteManager(object):
   ...
   class _SM(object):
      ...
      def getSize(self, fileName):
         spriteSize = SpriteManager._SM._SPRITE_SIZES.get(fileName, SpriteManager._SM._DEFAULT_SPRITE)
         return spriteSize
      
      def getSprite(self, fileName, offset=None, scale=None):
      ...
      def _loadImage(self, fileName, sheet=False):
         ...
         if sheet:
            ...
            scaleFactor = SpriteManager._SM._SPRITE_SCALES.get(fileName, 1.0)
            
            # See how big the sprite sheet is
            sheetDimensions = fullImage.get_size()

            scaledSize = (int(spriteSize[0] * scaleFactor),
                          int(spriteSize[1] * scaleFactor))
            ...
            for y in range(0, sheetDimensions[1], spriteSize[1]):
               ...
               for x in range(0, sheetDimensions[0], spriteSize[0]):
                  ...
                  if scaleFactor != 1.0:
                     sprite = pg_scale(sprite, scaledSize)
